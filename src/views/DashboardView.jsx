  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title="Daily Revenue" value={`RM ${(kpis?.dailyRevenue || 0).toLocaleString()}`} change={kpis?.dailyRevenueChange} icon={DollarSign} delay={0} />
        <KPICard title="Inventory Health" value={`${kpis?.inventoryHealth || 0}%`} change={kpis?.inventoryHealthChange} icon={Package} delay={0.05} />
        <KPICard title="Customer Sentiment" value={`${kpis?.customerSentiment || 0}/5`} change={kpis?.customerSentimentChange} icon={Heart} delay={0.1} />
        <KPICard title="Waste Reduction" value={`${kpis?.wasteReduction || 0}%`} change={kpis?.wasteReductionChange} icon={TrendingDown} delay={0.15} />
      </div>
